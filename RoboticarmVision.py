...
import matplotlib # matplot lib for the graph report 
import csv  # Writing the csv back 
...
def Kinematicframework_Algorithm(BeaconRSSI,x,y,z,AngleW,speed,timing,GearratioS,GearratioB): # Robotic positioning and and location track 
  ...
  #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>             #Angle converter  
     AngleEl = 63.72*stepE/64  # Angle Elbow converter
     AngleS = 63.72*stepS/GearratioS #Angle Elbow converter 
     AngleB = 63.72*stepB/GearratioB #Angle Base converter
  #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   ...
   # Y lenght   Arm
    ...
    dx = dy*math.cos(math.radians(AngleB)) # Angle base output 
    AngleO = math.atan(dy/dx)
    # calculate the angle for the Origin angle cosine  
    dz = dy*math.sin(math.radians(AngleB+AngleO)) # Angle Shoulder for calculating hight  
    print("Axis output x,y,z",dx,dy,dz)  # diplay output distance  
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>               
